...
function QuizModal({ quizzes, courseId, quizSetId, open, setOpen }: QuizModalProps) {
  ...
  }, [quizzes, courseId, quizSetId, open]);

  // ✅ Reset quiz index cuando se abre el modal
  useEffect(() => {
    if (open) {
      setQuizIndex(0);
      setAnswers([]);
      console.log("🚀 QuizModal opened, reset to first quiz");
    }
  }, [open]);

  // ✅ CORRECCIÓN: Validación más robusta
  if (!quizzes || quizzes.length === 0) {
    console.log("⚠️ QuizModal: No quizzes provided");
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[95%]">
          <DialogTitle>Quiz no disponible</DialogTitle>
          <div className="p-4">
  ...
  // ✅ CORRECCIÓN: Validación del quiz actual
  if (!currentQuiz) {
    console.error("❌ Current quiz not found at index:", quizIndex);
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[95%]">
          <DialogTitle>Error en el Quiz</DialogTitle>
          <div className="p-4">
            <p>Error al cargar la pregunta actual.</p>
          </div>
          <DialogFooter>
            <Button onClick={() => setOpen(false)}>Cerrar</Button>
          </DialogFooter>
  ...
  const quizChangeHandler = (type: "next" | "prev") => {
    console.log("🔄 Quiz navigation:", type, "current index:", quizIndex);
  ...
  const updateAnswer = (
    ...
    // Filtrar respuestas anteriores para este quiz y agregar la nueva
  ...
  console.log("🎯 QuizModal rendering:", {
    open,
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[95%] max-h-[90vh] overflow-y-auto">
        ...
        <div className="py-4">
          ...
          <span className="text-[10px] block text-end text-gray-500 mb-6">
            <svg
              stroke="currentColor"
              fill="currentColor"
              strokeWidth="0"
        ...
        <div className="grid md:grid-cols-2 gap-5 mb-6">
          {currentQuiz.options?.map((option, index) => (
            <div key={`${currentQuiz.id}-${option.label}-${index}`}>
              <input
